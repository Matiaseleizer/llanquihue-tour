![Duoc UC](https://www.duoc.cl/wp-content/uploads/2022/09/logo-0.png)
# 🧠 Manejando colecciones y estructuras reutilizables S4 – Programacion Orientado a Objetos I

## 👤 Autor del proyecto
- **Nombre completo:** [Matias Ignacio Altamirano Moreno]
- **Sección:** [PRY2202]
- **Carrera:** Analista Programador computacional
- **Sede:** [Campus Virtual]

---

## 📘 Descripción general del sistema
Una aplicación de consola en Java diseñada para cargar, gestionar y filtrar un catálogo de actividades y centros turísticos. Este proyecto pone en práctica conceptos de Programación Orientada a Objetos (POO), manejo de archivos de texto y manipulación de colecciones.

---

## 🧱 Estructura general del proyecto

El proyecto está organizado en los siguientes paquetes:

* **`ui`**: Contiene la clase principal para la ejecución del sistema.
    * `Main.java`: Punto de entrada del programa.
* **`model`**: Contiene la clase CentroTuristico la cual le da los atributos a los objetos.
    * `CentroTuristico.java`: Clase base general.
* **`data`**: Clase integrada mediante composición.
  * `GestorDatos.java`: Clase que contiene al array y lee los datos.
* **`resources`**: Contiene el archivo .txt con los datos que se trabajaron.

---

## ⚙️ Instrucciones para clonar y ejecutar el proyecto

1. Clona el repositorio desde GitHub:

```bash
https://github.com/Matiaseleizer/LlanquihueTour.git
```

2. Abre el proyecto en IntelliJ IDEA.

3. Verifica que los archivos `.txt` estén correctamente ubicados.

4. Ejecuta el archivo `Main.java` desde el paquete `app`.

5. Sigue las instrucciones en consola o en la interfaz gráfica (si corresponde).


---

**Repositorio GitHub:** \[https://github.com/Matiaseleizer/llanquihue-tour.git]
**Fecha de entrega:** \[12/06/2026]

---

© Duoc UC | Escuela de Informática y Telecomunicaciones




